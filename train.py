...
def validation(generator, validation_loader, sw, h, steps, device, first=False):
    ...
    with torch.no_grad():
        for j, batch in enumerate(validation_loader):
            ...
            y_g_hat_mel = mel_spectrogram(y_g_hat.squeeze(1), h.n_fft, h.num_mels, h.output_sampling_rate,
            ...
            if j <= 4:
                if steps == 0 or first:
                    sw.add_audio('gt/y_{}'.format(j), y[0], steps, h.output_sampling_rate)
                    sw.add_figure('gt/y_spec_{}'.format(j), plot_spectrogram(x[0]), steps)

                sw.add_audio('generated/y_hat_{}'.format(j), y_g_hat[0], steps, h.output_sampling_rate)
                y_hat_spec = mel_spectrogram(y_g_hat.squeeze(1), h.n_fft, h.num_mels,
                                                h.output_sampling_rate, h.hop_size, h.win_size,
                                                h.fmin, h.fmax)
...
def train(rank, a, h):
    ...
    trainset = MelDataset(training_filelist, h.segment_size, h.n_fft, h.num_mels,
                          h.hop_size, h.win_size, h.output_sampling_rate, h.input_sampling_rate, h.fmin, h.fmax, n_cache_reuse=0,
                          shuffle=False if h.num_gpus > 1 else True, fmax_loss=h.fmax_for_loss, device=device,
                          mel_spec_fine_tuning=a.mel_spec_fine_tuning, base_mels_path=a.input_mels_dir, noise_addition=h.noise_addition, config_noise_aug=h.noise_aug_params)
    ...
    if rank == 0:
        validset = MelDataset(validation_filelist, h.segment_size, h.n_fft, h.num_mels,
                              h.hop_size, h.win_size, h.output_sampling_rate, h.input_sampling_rate, h.fmin, h.fmax, False, False, n_cache_reuse=0,
                              fmax_loss=h.fmax_for_loss, device=device, mel_spec_fine_tuning=a.mel_spec_fine_tuning,
    ...
    for epoch in range(max(0, last_epoch), a.training_epochs):
        ...
        for i, batch in enumerate(train_loader):
            ...
            y_g_hat = generator(x)
            y_g_hat_mel = mel_spectrogram(y_g_hat.squeeze(1), h.n_fft, h.num_mels, h.output_sampling_rate, h.hop_size, h.win_size,
                                          h.fmin, h.fmax_for_loss)
...
def main():
    ...
    parser.add_argument('--mel_spec_fine_tuning', default=False, type=bool)
